...
public class TopoLogicalSort
{
	...
	private void DFSVisit(Graph graph, Vertex u, List<Vertex> result)
	{
		...
		Set<String> list = u.getAdjMap().keySet();
		for (String s : list)// explore edge(u,v)
		{
			Vertex v = graph.search(s);
		...
		time++;
		u.setFinish(time);
		result.add(0, u);// as each vertex is finished,insert it onto the front of a linked list
	}
}
